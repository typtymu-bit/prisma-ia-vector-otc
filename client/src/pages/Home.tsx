        ...
        <section className="min-w-0 flex-1">
          <div className="border-b border-white/8 bg-[#0b1120]/35 px-4 py-2 lg:hidden">
            <div className="flex gap-2 overflow-x-auto">
              ...
              {assets.map((asset) => <button key={asset.id} onClick={() => setActiveId(asset.id)} className={`shrink-0 rounded-lg px-2.5 py-2 text-xs ${asset.id === activeAsset?.id ? "bg-violet-400/15 text-violet-100" : "text-slate-500"}`}>{asset.label.replace(" OTC", "")}</button>)}
            </div>
          </div>
  ...
  return <div className="p-4 sm:p-6">
    <section className="flex flex-col justify-between gap-4 rounded-2xl border border-white/8 bg-[#10182a]/75 p-4 shadow-2xl shadow-black/10 xl:flex-row xl:items-center">
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
        <Metric label="PREÇO AO VIVO · 1S" value={formatPrice(livePrice)} accent="text-white" />
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_350px]">
      <div className="min-w-0 space-y-4">
        <section className="overflow-hidden rounded-2xl border border-white/8 bg-[#10182a]/75 shadow-2xl shadow-black/10">
          ...
          <div className="grid gap-px border-t border-white/8 bg-white/5 sm:grid-cols-4"><LineLegend color="bg-orange-400" title="SUPORTE 1 · LARANJA" value={formatPrice(analysis?.levels.support)} /><LineLegend color="bg-sky-400" title="SUPORTE 2 · AZUL" value={formatPrice(analysis?.levels.supportOuter)} /><LineLegend color="bg-amber-300" title="RESISTÊNCIA 1" value={formatPrice(analysis?.levels.resistance)} /><LineLegend color="bg-fuchsia-300" title="RESISTÊNCIA 2" value={formatPrice(analysis?.levels.resistanceOuter)} /></div>
        </section>
...
function TradingChart({ candles, analysis, emptyMessage }: { candles: Candle[]; analysis: VectorAnalysis | null; emptyMessage: string }) {
  ...
  const values = [...visible.flatMap((candle) => [candle.high, candle.low]), analysis?.levels.support ?? NaN, analysis?.levels.supportOuter ?? NaN, analysis?.levels.resistance ?? NaN, analysis?.levels.resistanceOuter ?? NaN].filter(Number.isFinite);
  ...
  return <div className="relative overflow-hidden rounded-xl border border-white/6 bg-[#090e19]"><svg viewBox={`0 0 ${width} ${height}`} className="block h-auto w-full" role="img" aria-label="Gráfico real de velas de 1 minuto com quatro níveis horizontais">{[0.2, 0.4, 0.6, 0.8].map((ratio) => <line key={ratio} x1={pad.left} x2={width-pad.right} y1={pad.top + chartH * ratio} y2={pad.top + chartH * ratio} stroke="rgba(148,163,184,.11)" strokeDasharray="4 5" />)}{analysis?.levels && <><line x1={pad.left} x2={width-pad.right} y1={y(analysis.levels.support)} y2={y(analysis.levels.support)} stroke="#fb923c" strokeWidth="2" strokeDasharray="8 5" /><line x1={pad.left} x2={width-pad.right} y1={y(analysis.levels.supportOuter)} y2={y(analysis.levels.supportOuter)} stroke="#38bdf8" strokeWidth="1.8" strokeDasharray="4 5" /><line x1={pad.left} x2={width-pad.right} y1={y(analysis.levels.resistance)} y2={y(analysis.levels.resistance)} stroke="#fbbf24" strokeWidth="2" strokeDasharray="8 5" /><line x1={pad.left} x2={width-pad.right} y1={y(analysis.levels.resistanceOuter)} y2={y(analysis.levels.resistanceOuter)} stroke="#f0abfc" strokeWidth="1.8" strokeDasharray="4 5" /><text x={width-pad.right-4} y={y(analysis.levels.support)-4} textAnchor="end" fill="#fb923c" fontSize="9">S1</text><text x={width-pad.right-4} y={y(analysis.levels.supportOuter)-4} textAnchor="end" fill="#38bdf8" fontSize="9">S2</text><text x={width-pad.right-4} y={y(analysis.levels.resistance)-4} textAnchor="end" fill="#fbbf24" fontSize="9">R1</text><text x={width-pad.right-4} y={y(analysis.levels.resistanceOuter)-4} textAnchor="end" fill="#f0abfc" fontSize="9">R2</text></>}{visible.map((candle, index) => { const rising = candle.close >= candle.open; const color = rising ? "#50e3a4" : "#fb7185"; const center = x(index); const top = y(Math.max(candle.open, candle.close)); const bodyHeight = Math.max(1.5, Math.abs(y(candle.open)-y(candle.close))); return <g key={candle.time}><line x1={center} x2={center} y1={y(candle.high)} y2={y(candle.low)} stroke={color} strokeWidth="1.2" opacity=".9" /><rect x={center-candleWidth/2} y={top} width={candleWidth} height={bodyHeight} rx="1" fill={color} /></g>; })}{analysis?.signalReady && <g transform={`translate(${x(visible.length - 2)},${y(visible[visible.length - 2].high) - 18})`}><circle r="11" fill={analysis.direction === "call" ? "#34d399" : "#fb7185"} opacity=".95" /><path d={analysis.direction === "call" ? "M-4,3 L0,-4 L4,3" : "M-4,-3 L0,4 L4,-3"} fill="none" stroke="#07111f" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></g>}</svg><div className="absolute left-3 top-3 rounded-md border border-white/10 bg-[#0b1120]/85 px-2 py-1 font-mono text-[10px] text-slate-500">60 velas reais · 1 minuto</div></div>;
}
...
function Scanner({ assets, onSelect }: { assets: OtcAsset[]; onSelect: (id: number) => void }) {
  ...
  const run = () => scan.mutate({ activeIds: assets.map((asset) => asset.id) });
